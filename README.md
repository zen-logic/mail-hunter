...
- **IMAP Sync** -- Incremental sync via UID checkpoints. Full resync and purge options. Sync queue ensures one operation at a time. Progress streamed live over WebSocket.
...
- **Deduplication** -- Messages hashed by Message-ID and content. Identical messages across servers and folders stored once on disk.
...
- **Search** -- Full-text search across all servers, folders, and years. Filter by sender, recipient, subject, body, date range, tags, or legal hold status.
- **Batch Operations** -- Select multiple messages with click, shift-click, or select-all. Batch delete, batch tag, batch hold. Legal hold respected across all bulk operations.
